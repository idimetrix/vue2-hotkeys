...
const getKeyMap = (keymap, alias = {}) =>
  Object.keys(keymap).map(input => {
    const result = {};
...
const searchKeyCode = key => {
  ...
  return (
    codes[key.toLowerCase()] ||
    aliases[key.toLowerCase()] ||
    returnCharCode(key)
  );
};

const returnCharCode = key =>
...
const hasKeyCode = key =>
  !isPlainObject(key) ? key : key.which || key.keyCode || key.charCode || false;

export { aliases, codes, getKeyMap };
